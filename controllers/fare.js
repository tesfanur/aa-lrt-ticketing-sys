...
var FareModule = (function(FareDal) {
    ...
    if (validSourceId && validDestinationId) {
      ...
      FareDal.fareExist(from, to)
        .then(result => {
          if (result) {
            res.status(400).send({
              "message": "Fare from " + from + " to " + to + " already exists"
            });
          } else {
      ...
      res.status(400).send({
        "Message": "Source or Destination Id is invalid"
      });
    }
  }
  /**
  ...
  function setFareAmount() {
    ...
    FareDal.setFareAmount(body.from.body.to, body.fare)
      .then(result => {
        ...
        res.status(500).send({
          "MESSAGE": "UNABLE TO SET FARE AMOUNT"
        });
      })
      .catch(err => {
  ...
  function setDistance() {
    ...
    FareDal.setFareAmount(body.from.body.to, body.distance)
      .then(result => {
        if (result) return res.send(result);
        res.status(500).send({
          "MESSAGE": "UNABLE TO SET   DISTANCE"
        });
      })
      .catch(err => {
        res.status(500).send(err)
  ...
  function getAllFares(req, res, next) {
    FareDal.getAll({})
      .then(fares => {
        if (fares) fares = {
          "total number of fare documents": fares.length,
          fares
        }
