...
function populateFareCollection(createdBy, fares) {
            ...
            if (!result || result.length === 0){
                ...
                FareModel.collection.insert(fares, (error, docs) => {
                  if (error) {
                    ...
                    console.info('%d fares successfully stored.', docs.length);
                  }
                });
              }
              ...
              FareModel.collection.updateMany({}, {
                $set: {
                  createdAt: createdAt,
    ...
    function generateTicketInfo(route, from, to, user) {
      ...
      return new Promise((resolve, reject) => {
        ...
        StationDal.findByCustomId(parseInt(from))
          .then(sourceStation => {
            console.log("sourceStation:", sourceStation.nameEng);
            return sourceStation;
          })
          .then((sourceStation) => {
            StationDal.findByCustomId(parseInt(to))
              .then(destinationStation => {
                console.log("destinationStation:", destinationStation.nameEng);
                //=================================================
                var source = from;
                var destination = to;
