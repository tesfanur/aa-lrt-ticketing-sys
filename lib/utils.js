...
var handleResponse=(res,status=200,doc)=>{
    if(!doc || doc===404) return res.status(status).send({"message":"No maching document found."});
...
var handleErrors = (err, req, res, next)=>{
	...
	res.json({ "error message": err.message + ' Please type the address properly',
...
/**
*Custom error handling function//to be removed
*/
var handleError = function (res, err, obj){
